...
const TaskForm = ({ tasks, setTasks, editingTask, setEditingTask }) => {
  const { user } = useAuth();
  const [formData, setFormData] = useState({ title: '', description: '', deadline: '', plate: '', location: '', fine: '' });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setEditingTask(null);
      setFormData({ title: '', description: '', deadline: '', plate: '', location:'', fine:''});
    } catch (error) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-6 shadow-md rounded mb-6">
      <h1 className="text-2xl font-bold mb-4">{editingTask ? 'Edit Violation' : 'Record a Violation'}</h1>
      <input
        type="text"
